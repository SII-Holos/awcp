...
interface InviteMessage extends BaseMessage {
  ...
  requirements?: {
    transport?: "sshfs" | "archive" | "storage";  // 期望的传输方式
  };
...
> **说明**：`environment.resources` 支持声明多个资源，但 v1 参考实现通常只使用一个名为 `"workspace"` 的 `fs` 资源。`include/exclude` 字段在类型定义中保留，但 v1 参考实现尚未启用过滤逻辑。
...
Storage 传输：
```typescript
interface StorageWorkDirInfo {
  transport: "storage";
  downloadUrl: string;      // Executor 下载工作空间的预签名 URL
  uploadUrl: string;        // Executor 上传结果的预签名 URL
  checksum: string;         // SHA-256 校验和
  expiresAt: string;        // URL 过期时间（ISO 8601 格式）
  headers?: Record<string, string>;  // 可选认证头
}
```
...
DONE 是异步消息，通过 SSE 事件流返回（而非 HTTP 响应），因为任务执行时间不确定。对应的事件结构为 `TaskDoneEvent`：

```typescript
interface TaskDoneEvent {
  delegationId: string;
  type: "done";             // 注意：SSE 事件中 type 为小写
...
> **说明**：SDK 内部会将 `TaskDoneEvent` 转换为 `DoneMessage`（`type: "DONE"`，`finalSummary` 字段）用于状态记录，但网络传输的始终是 SSE 事件格式。
...
数据面负责将 Delegator 的 Environment Directory 映射到 Executor 的 Work Directory。AWCP v1 通过 **Transport Adapter** 抽象支持多种传输方式，目前提供两种实现：**SSHFS**（挂载式）和 **Archive**（归档式）。两种传输方式各有优劣，适用于不同场景。
...
```typescript
{
  delegator: {
    provider: {
      type: "local",           // 或 "s3"
      endpoint: "http://...",  // 存储服务 URL
      localDir: "/path/to/storage"
    },
    tempDir: "~/.awcp/temp"
  }
}
...
class DelegatorService {
  constructor(options: {
    config: {
      environment: { baseDir: string };   // 环境目录根路径
  ...
  delegate(params: {
    executorUrl: string;
    environment: EnvironmentSpec;         // 环境资源声明
...
当使用 Archive 传输时，任务完成后 Delegator 会收到 `resultBase64` 字段，包含修改后工作空间的 ZIP 归档。SDK 会自动将其应用回原始资源目录：
...
> **实现说明**：`delegate` 工具是便捷封装，内部会将 `workspace_dir` 转换为单个 `fs` 资源：`environment.resources = [{ name: "workspace", type: "fs", source: workspace_dir, mode: access_mode }]`。
