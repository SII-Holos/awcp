...
| **Transport Adapter（传输适配器）** | 数据面的可插拔实现。负责工作空间的实际传输，v1 提供 SSHFS、Archive 和 Storage 三种适配器。 |
...
**语义**：Delegator 发起协作请求。包含任务描述、环境资源声明与约束建议，**不含传输凭证或工作空间数据**。`environment` 字段仅为元数据声明，用于 Executor 了解资源配置并进行预检，实际的工作空间数据在 START 阶段通过 `workDir` 传输。
