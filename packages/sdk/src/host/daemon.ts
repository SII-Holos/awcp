 ...
 * Host Daemon
 * 
 * Manages delegations from the Host (Delegator) side.
 * Responsible for:
 ...
 */
export class HostDaemon extends EventEmitter {
  ...
  private config: HostDaemonConfig;

  constructor(config: HostDaemonConfig) {
    super();
    this.config = config;
   ...
   * 
   * This is the main entry point for Host Agent to initiate collaboration.
  ...
  async handleMessage(message: AwcpMessage): Promise<void> {
    ...
    switch (message.type) {
      ...
      case 'ERROR':
        ...
        break;
      default:
        console.warn(`Unexpected message type from Remote: ${message.type}`);
    }
  }
  ...
  async cancelDelegation(delegationId: string): Promise<void> {
    ...
    const errorMessage: ErrorMessage = {
      ...
      message: 'Delegation cancelled by Host',
    };
