...
export interface RemoteDaemonConfig {
  ...
  /** Callback to mount remote filesystem */
  mount: (params: {
...
/**
 * Events emitted by RemoteDaemon
 */
export interface RemoteDaemonEvents {
...
/**
 * Active delegation state on Remote side
 */
 ...
 * Remote Daemon
 * 
 * Manages delegations from the Remote (Collaborator) side.
 * Responsible for:
 * - Receiving and accepting invitations
 * - Enforcing local security policy
 * - Mounting remote workspaces
 * - Executing tasks
 * - Reporting results
 */
export class RemoteDaemon extends EventEmitter {
  ...
  constructor(config: RemoteDaemonConfig) {
    super();
    this.config = config;
  ...
  async acceptInvitation(
    delegationId: string,
    constraints?: Partial<RemoteConstraints>,
  ): Promise<void> {
    ...
    // Step 4: Build remote constraints
    const remoteConstraints: RemoteConstraints = {
