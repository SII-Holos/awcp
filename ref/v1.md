...
*   `requirements` (Remote 预检项):
    *   `mount_transport`: `"sshfs"` (可选，默认 sshfs；Remote 可据此检查本机是否具备对应挂载能力)
...
v1 协议中，`mount_transport` 默认值为 `"sshfs"`。
...
### 6.3 Git 审计增强 (Optional Git Audit)
...
### 6.4 并发控制策略 (Concurrency Strategy)
...
3.  **Control**
    *   `cancel_delegation(id)`: 强制中止任务，立即回收 Export View 与凭证，发送 `CANCEL` 信号。
...
*   **参数映射**：
    ...
    *   `workspace_dir` → `INVITE.workspace` (用于创建 Export View)
...
*   **映射关系**：触发 Daemon 的 `cancel_delegation` 流程（发送 `CANCEL` 消息、清理挂载）。
