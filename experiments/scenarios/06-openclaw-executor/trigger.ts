/**
 * Trigger script - Tests MCP + OpenClaw Executor integration
 *
 * This simulates how an AI Agent would use the AWCP MCP tools to delegate
 * coding tasks to an OpenClaw-based executor.
 *
 * Key features:
 * - Uses openclaw-executor which processes tasks with OpenClaw AI
 * - Supports various model providers (DeepSeek, Anthropic, OpenAI, OpenRouter)
 * - Tests real code modification capabilities
 */
...
async function main() {
  console.log('');
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║     OpenClaw Executor MCP Integration Test                 ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  const client = new Client(
    { name: 'openclaw-executor-test', version: '1.0.0' },
    { capabilities: {} }
  );
...
async function testListTools(client: Client) {
  ...
  try {
    ...
    // Verify agent card info was injected (OpenClaw Executor)
    const hasAgentName = description.includes('OpenClaw');
    const hasPeerUrl = description.includes(EXECUTOR_BASE_URL);
    const hasSkills = description.toLowerCase().includes('skill');
    const hasCapabilities = description.toLowerCase().includes('code') || 
                           description.toLowerCase().includes('execution') ||
                           description.toLowerCase().includes('review');
    
    console.log('  Agent Card injection check:');
    console.log(`    - Agent name "OpenClaw": ${hasAgentName ? GREEN + '✓' : RED + '✗'}${NC}`);
...
async function testDynamicFileTask(client: Client) {
  const testName = 'Dynamic file modification via OpenClaw';
  log(BLUE, '\n[TEST]', testName);
  ...
  try {
    log(YELLOW, '  →', 'Calling delegate tool with dynamic OpenClaw task...');
...
`.trim();
    ...
    if (!wasModified) {
      log(YELLOW, '!', 'File was not modified (OpenClaw may have interpreted the task differently)');
    } else {
      log(GREEN, '✓', 'File was successfully modified by OpenClaw');
    }
